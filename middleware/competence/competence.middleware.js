...
async function validateCodeUniqueness(req, res, next) {
    ...
    const existingCode = await Competence.findOne({ program_code });
    if (existingCode) {
        console.log('existe')
        apiStructure.setStatus("Failed", 400, `Lo sentimos, el código de competencia laboral '${program_code}' ya está en uso. Por favor, elija un código único.`);
...
const validateCompetence = async (req, res, next) => {
    ...
    const expectedFields = ["labor_competition", "labor_competence_code", "competition_name", "labor_competition_version", "maximum_duration", "quarter", "program" ];
